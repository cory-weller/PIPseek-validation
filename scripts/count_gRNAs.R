...
fastq_file <- 'data/DR-9163_25_S25_L001_R2_001.fastq.gz'
gRNA_file <- 'data/adt-tags-183sgRNAsequences-Tian2019.csv'
command <- paste0('zcat ', fastq_file, " | awk 'NR%4==2'")
...
# Load in gRNA csv
expected_gRNA <- fread(gRNA_file, header=FALSE)
...
ggsave(g, file='plots/gRNA_distribution_log10.png', width=50, height=8, units='cm')

# Export linear scale
g <- ggplot(dat[N>1], aes(x=ID, y=N)) + geom_point() +
theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
ggsave(g, file='plots/gRNA_distribution_linear.png', width=50, height=8, units='cm')